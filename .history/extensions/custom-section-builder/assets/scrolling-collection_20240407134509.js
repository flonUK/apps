document.addEventListener('DOMContentLoaded', function() {
  ...
  if (isDesktop) {
    ...
    columns.forEach(column => {
      const products = column.querySelectorAll('.adedayo-scrolling-collection-product-card');
      const firstProduct = products[0].cloneNode(true);
      const lastProduct = products[products.length - 1].cloneNode(true);
      column.insertBefore(lastProduct, products[0]);
      column.appendChild(firstProduct);
    });
