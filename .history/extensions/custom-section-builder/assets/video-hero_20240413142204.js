    ...
    if (video) {
      if (video.tagName === 'VIDEO') {
        ...
        if (video.hasAttribute('autoplay')) {
          video.play().then(function() {
            // Hide the poster, play button, and overlay on successful autoplay
            if (poster) {
              poster.style.opacity = '0';
              setTimeout(function() {
                poster.style.display = 'none';
          ...
          // If autoplay is not enabled, make sure the cover image, play button, and overlay are visible
          if (poster) {
            poster.style.display = 'block';
            poster.style.opacity = '1';
          }
