...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function playVideo(videoWrapper) {
    ...
    const overlay = videoWrapper.closest('.adedayo-video-hero').querySelector('.adedayo-video-hero-overlay');
    ...
    if (video) {
      if (video.tagName === 'VIDEO') {
        ...
        video.play().then(function() {
          // Hide the poster, play button, and overlay on successful play
          if (poster) {
        ...
        // Hide the poster, play button, and overlay
        if (poster) {
          poster.style.opacity = '0';
          setTimeout(function() {
            poster.style.display = 'none';
