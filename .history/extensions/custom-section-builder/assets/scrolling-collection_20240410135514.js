document.addEventListener('DOMContentLoaded', function() {
    ...
    function autoScrollColumns(columns) {
      columns.forEach((column, index) => {
        // Determine the initial direction; the left column scrolls upwards, the right scrolls downwards.
        ...
        function scroll() {
          ...
          if (scrollDirection > 0 && currentScroll >= maxScroll) {
            // Downwards: When reaching the bottom, reset to the top.
            column.scrollTop = 0;
          } else if (scrollDirection < 0 && currentScroll <= 0) {
            // Upwards: If attempting to scroll above the top, reset to the bottom.
        ...
        column.addEventListener('mouseleave', () => {
          ...
          requestAnimationFrame(scroll); // Ensure scrolling resumes immediately after mouse leave
        });
      });
    }
  
    // Mobile rows' scrolling logic (unchanged from last correction)
