document.addEventListener('DOMContentLoaded', function() {
    ...
    const scrollSpeed = parseInt(document.querySelector('.collection-grid').dataset.scrollSpeed, 10);
    ...
    if (isDesktop) {
      ...
      const columns = document.querySelectorAll('.collection-grid-products-column');
      // Start the auto-scroll for desktop columns
      autoScrollColumns(columns);
    } else {
      // Mobile scroll logic
      const topRow = document.querySelector('.collection-grid-products-row:nth-child(1)');
      const bottomRow = document.querySelector('.collection-grid-products-row:nth-child(2)');
