document.addEventListener('DOMContentLoaded', function() {
  ...
  function scrollProductSpotlight() {
    currentPosition -= cardWidth;
    productSpotlight.style.transform = `translateX(${currentPosition}px)`;

    if (currentPosition <= -productSpotlight.scrollWidth + productSpotlight.offsetWidth) {
      currentPosition = 0;
      productSpotlight.style.transform = 'translateX(0)';
    }
  }
  ...
  imageContainers.forEach(function(imageContainer) {
    ...
    if (transition === 'fade') {
      ...
      if (images.length > 1) {
        interval = setInterval(scrollProductSpotlight, duration);
      }
    }
